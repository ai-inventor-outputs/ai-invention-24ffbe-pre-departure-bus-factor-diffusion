...
```
model: claude-sonnet-5 | effort: low | permission: bypassPermissions | cwd: /ai-inventor/aii_data/runs/run_5SMkWpWKNLxk/3_invention_loop/iter_2/gen_plan/gen_plan_dataset_1
```

### [2] SYSTEM-USER prompt · 2026-08-20 20:08:33 UTC
...
type: dataset
objective: >-
  Attempt to build a founder-departure commit corpus sampled from a historical snapshot that does NOT condition on present-day
  liveness, using the research artifact's recommended source and the existing ~104-repo candidate pipeline/checkpoint as a
  starting point, in order to give the experiment/evaluation a chance at a non-surviving founder-only TFDD event and a genuinely
  non-survivorship-biased corpus.
approach: >-
  First check whether a GITHUB_TOKEN or equivalent authenticated credential is available in this environment; if so, resume
  the existing checkpointed 104-repo candidate pipeline (the prior dataset artifact's code/build_dataset.py, code/candidates.py,
  temp/checkpoint.json, reused by re-reading that workspace's code) using it to raise the rate limit and pull full commit
  histories for as many additional candidates as the time budget allows, explicitly including repositories that are NOT currently
  famous/thriving (e.g. sampled from older, less-starred, or now-quiet repos in the candidate list) to avoid re-introducing
  the same survivorship bias. If no token is available, pull instead from GH Archive or another non-authenticated historical
  source identified by the research artifact: construct a sample of repositories that existed and were active in a fixed past
  window (e.g. repos with commit activity in a chosen year several years ago), explicitly NOT filtered by whether they are
  still maintained today, then `git clone` + `git log --numstat` each (unrated-limited) to get full commit history exactly
  as the existing dataset does. Standardize output to the same exp_sel_data_out schema as the existing dataset artifact (repo_metadata,
  founder_signal, commits[]) so it is a drop-in-compatible superset/companion corpus, and explicitly report per-repo whether
  each was sampled by a liveness-conditioned or liveness-non-conditioned procedure via a new sampling_frame metadata field,
  so downstream analysis can compare or pool them honestly rather than silently mixing biased and unbiased samples.
depends_on: []
</artifact_direction>
